...
- **17/8**: Recalibrate lại `content_threshold`/ngưỡng theo phân phối điểm Branch 2 mới (sau khi B2 đổi feature/data) — `models/branch3_v2/metadata.json` cập nhật `content_threshold=0,3383`.
- **10/8 (PR #20, đã merge)**: Dọn sạch artifact của thiết kế GRU cũ đã bị thay thế, tránh nhầm lẫn thiết kế nào đang thật sự chạy — xoá `_GRUClassifier`/`SessionSequenceDetector` khỏi `src/models/branch3_session.py`, xoá `sessions_to_arrays()`/`evaluate_sessions()` khỏi `src/models/branch3_features.py` và test GRU tương ứng, xoá `models/branch3_v1/` (weight GRU cũ) và `data/raw/branch3_sessions/` (~25MB, data mồ côi từ nhánh `feature/branch3-multi-field` chưa từng merge). Cập nhật README/AGENTS.md/`deploy/README.md`/`app/README.md` không còn mô tả Branch 3 là `not_ready`.
- **13/8 (audit của Bách, PR #21, merge 15/8) — phát hiện quan trọng, làm sai lệch tuyên bố "tách biệt hoàn toàn train" ở dòng "Kết quả thật" phía trên**: `branch3_sessions_cach_a.csv` dùng chung **1 pool 100 user cố định** cho cả TRAIN và TEST; vì thuật toán bisection là deterministic theo từng user mục tiêu, **65/70 session `boolean_blind` và 67/70 session `time_blind` ở TEST giống byte-for-byte với 1 session nào đó ở TRAIN**. Tức là FPR=0,0/DR=1,0 báo cáo trước đó phản ánh việc mô hình **ghi nhớ** một pool đóng nhỏ, chưa chắc là khả năng tổng quát hoá thật. `query_splitting` (0/70 trùng) và `benign` (0/70 trùng dù share target) sạch, không bị ảnh hưởng. Chi tiết đo đạc: `report/metrics/audit_branch3/audit_report.md`, `findings.json` (chạy lại bằng `train/audit_branch3_data_validity.py`).
  - Hướng xử lý: **chưa fix** (chưa mở rộng pool user hoặc đổi thuật toán sinh session để phá tính deterministic) — cần nêu rõ giới hạn này ở phần Limitations của bài báo thay vì tiếp tục trình bày FPR=0,0/DR=1,0 như một kết quả tổng quát hoá không điều kiện.
- **19/8**: Recalibrate lại lần nữa theo phân phối điểm Branch 2 sau khi B2 chuyển sang LocalOutlierFactor (mục 3) — `content_threshold` giữ nguyên 0,3383 (trùng ngẫu nhiên với lần hiệu chỉnh 17/8), FPR=0,0/DR=1,0 trên benchmark hiện tại không đổi (vẫn mang giới hạn leakage nêu trên).
- **20/8 — đã fix leakage nêu ở mục 13/8, không còn "chưa fix" nữa.** Nguyên nhân sâu: `train/build_session_dataset.py` random hoá đúng 100-user pool để sinh target cho `boolean_blind`/`time_blind`, nhưng việc **chia TRAIN/TEST làm SAU khi sinh xong** (shuffle theo `session_id`), không quan tâm session nhắm vào user nào — nên cùng 1 user hoàn toàn có thể vừa làm target cho 1 session TRAIN vừa cho 1 session TEST, mà bisection deterministic thì 2 session đó y hệt nhau. Fix: tách pool 100 user thành 2 tập **rời nhau** (80 target-TRAIN / 20 target-TEST) **trước khi** sinh session, chỉ áp dụng cho `boolean_blind`/`time_blind` (2 lớp còn lại không deterministic theo target, không cần đổi). Regenerate `branch3_sessions_cach_a.csv`, chạy lại `train/audit_branch3_data_validity.py`: **byte-identical còn 0/70 cho cả 4 lớp** (trước: 65/70, 67/70). Recalibrate lại (`calibrate_branch3.py`, `eval_branch3_hard.py`): **FPR=0,0/DR=1,0 không đổi** trên data đã tách biệt thật — nghi vấn "ghi nhớ do rò rỉ" coi như bị bác bỏ bằng thực nghiệm, vì content-check nhận diện đúng *cú pháp* injection (không cần thấy target cụ thể trước đó). Caveat "Cách A tự sinh, chưa phải traffic tấn công độc lập" (chờ Cách B) vẫn còn nguyên giá trị, không liên quan đến bug này. `uv run pytest` xanh (248 test). Chi tiết: `report/plan/data_contract.md` §4.2 follow-up (20/8), `report/metrics/audit_branch3/audit_report.md` "Cập nhật (20/8)". Nhân tiện phát hiện thêm: cột "share target" trong audit script tự nó có bug regex (luôn khớp chuỗi giả `zzz` hardcode trong mọi payload, không phải target thật) — không đáng tin, chỉ cột "byte-identical" mới phản ánh đúng.
