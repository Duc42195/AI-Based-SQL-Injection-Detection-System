...
### (AI-Based SQL Injection Detection System) — Bản sửa đổi V6 (từ Ngày 5: Tôi=MLOps/API/Continual Learning, Bách=Train tất cả Nhánh 1+2+3, Minh=Streamlit, Diệp=Support)
...
**⚠️ Phân công lại từ Ngày 5 (17/7) — hoán đổi track giữa Tôi và Bách:**

| Vai trò | Ngày 1-4 | Ngày 5-14 (mới) |
|---|---|---|
| **Tôi** | Dữ liệu + Train Nhánh 1 | **Toàn bộ MLOps**: Bộ xử lý trung tâm, Session Store, đóng gói API (đang chủ động làm — `api/main.py`, `api/routers/`), benchmark, Continual Learning, Concept Drift |
| **Bách** | Chỉ Nhánh 2 | **Train tất cả**: giữ Nhánh 1+2 đã xong, **nhận thêm toàn bộ Nhánh 3** (Docker lab, sqlmap, session data, gán nhãn, chọn kiến trúc Tầng 2, train + đánh giá) |
| **Minh** | Streamlit | Streamlit (không đổi, chỉ đổi nguồn API tham chiếu theo chủ sở hữu mới) |
| **Diệp** | Support | Support (không đổi) |

**Lý do đổi:** Bách đã hoàn thành Nhánh 2 sớm và có kinh nghiệm pipeline train/eval, phù hợp nhận thêm Nhánh 3 (cùng bản chất: thu thập → gán nhãn → so sánh kiến trúc → train). Tôi đã chủ động bắt tay vào lớp API/hạ tầng — dồn hẳn về MLOps tận dụng đà đó, tránh chuyển ngữ cảnh qua lại giữa "train model" và "xây hạ tầng".
...
- *Tuần 1 (đã xong):* Tôi lo D1 → train Nhánh 1 → đánh giá. Bách lo D3 → train/đánh giá Nhánh 2 song song. Minh dựng khung Streamlit + trang mock. Diệp viết Mục 1-4 báo cáo.
- *Tuần 2 (kế hoạch mới):* Tôi xây Bộ xử lý trung tâm + Session Store + API tổng (Nhánh 1+2 trước) → benchmark → tích hợp Nhánh 3 khi Bách bàn giao → Continual Learning/Concept Drift. Bách dựng Docker lab + sqlmap → thu traffic → gán nhãn 2 tầng → chọn kiến trúc Tầng 2 (GRU/CNN/Transformer) → train + đánh giá Nhánh 3 → bàn giao cho Tôi tích hợp. Minh nối API thật ngay khi từng nhánh sẵn sàng (Nhánh 1+2 trước, Nhánh 3 sau ở Ngày 11). Diệp viết Mục 5-9 + tổng hợp báo cáo cuối.

---

## 12. Rủi ro sau khi đổi phân công (17/7)

**Bách giờ là đường găng chính (critical path) cho phần lõi 3 nhánh:** Nhánh 3 (Docker lab → sqlmap → gán nhãn → train) là track dài nhất (Ngày 5-10, 6 ngày), làm tuần tự, không ai san sẻ nếu trễ. Mốc kiểm tra quan trọng nhất: **Ngày 10 (Bách bàn giao model Nhánh 3 cho Tôi tích hợp)** — nếu trễ, việc tích hợp API + Bộ xử lý trung tâm 3 nhánh + benchmark toàn hệ thống (Ngày 11) đều trễ theo.

**Tôi (MLOps) ít phụ thuộc dữ liệu người khác hơn** — Bộ xử lý trung tâm, API, Session Store có thể xây trước bằng Nhánh 1+2 (đã có model thật), không cần chờ Nhánh 3. Rủi ro chính là khối lượng tích hợp dồn vào cuối (Ngày 11-13) khi Nhánh 3 vừa xong.

**Minh (Streamlit): vẫn không bị chặn** — nối API thật ngay khi có (Nhánh 1+2 đã sẵn sàng từ Ngày 4-7), chỉ chờ Nhánh 3 ở Ngày 11.

**Gợi ý giảm rủi ro:** nếu tới Ngày 8 Bách chưa xong thu thập traffic Nhánh 3, ưu tiên cắt bớt Cách B (sqlmap thật) và dùng Cách A (script mô phỏng, đã có sẵn ý tưởng từ trước) để không trễ toàn bộ track — chấp nhận giảm độ tin cậy dữ liệu Nhánh 3 hơn là trễ deadline.
