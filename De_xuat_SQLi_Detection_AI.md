...
|---|---|---|---|
| **HẠN NỘP** | **Thứ 7, 25/7** | Chỉ cần: **Nhánh 1 hoạt động mượt** (đã train, đang polish tích hợp) + **Nhánh 2 kiểm tra xong** (đã train lại, OCSVM AUC=0.90) + demo Streamlit end-to-end 2 nhánh + báo cáo | **Cắt hẳn khỏi phạm vi**: Nhánh 3 (session-level), Continual Learning, Concept Drift, Session Store, benchmark tải — toàn bộ chuyển vào **Future Work** (Mục 13) |
...
1. **[ĐÃ TRIỂN KHAI]** Nhánh 1 (supervised đa lớp, F1-macro=0.982) + Nhánh 2 (anomaly detection, OCSVM AUC=0.90) kết hợp trên cùng pipeline canonicalization, đóng gói API + demo thực tế.
...
**Phân công Ngày 9-13 (21-25/7) — CHỈ 5 NGÀY, scope tối thiểu:**

| Vai trò | Việc chính |
|---|---|
| **Tôi** | **Làm mượt Nhánh 1** (edge case canonicalization, ổn định API) + **Bộ xử lý trung tâm CHỈ 2 nhánh** (bỏ hẳn Nhánh 3 khỏi luồng thực tế) + test end-to-end + polish cuối |
| **Bách** | Verify kỹ kết quả Nhánh 2 vừa train lại (OCSVM: FPR=0,3%, detection rate=20,7%, AUC=0,90) — đủ số liệu đưa vào báo cáo chưa; hỗ trợ test edge case |
| **Minh** | Kết nối demo Streamlit với API thật Nhánh 1+2 (bỏ chỗ chờ Nhánh 3), polish UI, đảm bảo demo chạy mượt cho buổi nộp |
| **Diệp** | Viết lại dàn ý báo cáo theo scope 2 nhánh; Method + Kết quả thực nghiệm (số liệu thật) + Thảo luận/Hạn chế (nêu rõ Nhánh 3 là Future Work); hoàn thiện báo cáo cuối |
...
- *Ngày 9 (21/7, hôm nay):* Làm mượt Nhánh 1; phát hiện & sửa Nhánh 2 thiếu model (retrain lại chỉ mất 2,8 giây); viết lại dàn ý báo cáo theo scope 2 nhánh.
- *Ngày 10-11 (22-23/7):* Bộ xử lý trung tâm 2 nhánh + test end-to-end; demo Streamlit hoàn thiện; viết Method/Kết quả.
- *Ngày 12 (24/7):* Buffer sửa lỗi, polish cuối, hoàn thiện báo cáo.
- *Ngày 13 (25/7, THỨ 7 — HẠN NỘP):* Chốt demo + hệ thống + nộp báo cáo.
...
**Rủi ro còn lại — tích hợp/polish trong thời gian ngắn:** chỉ còn 5 ngày cho: làm mượt Nhánh 1, bộ xử lý trung tâm, demo end-to-end, viết báo cáo. Rủi ro chính là **dồn việc tích hợp + viết báo cáo vào cuối** nếu không làm song song từ Ngày 9. Đã giảm thiểu bằng cách giao Diệp viết báo cáo song song với Tôi/Minh làm kỹ thuật, không chờ tới khi có kết quả cuối cùng.

**Nhánh 2 không còn là rủi ro** — đã verify thực tế (21/7): toàn bộ pipeline build+train chạy trong ~75 giây. Việc còn lại chỉ là kiểm tra số liệu có đủ thuyết phục cho báo cáo, không phải chạy đua thời gian.

**Gợi ý nếu vẫn thiếu thời gian tới Ngày 12 (24/7):** ưu tiên cắt theo thứ tự — (1) bỏ polish UI kỹ càng, giữ demo chạy đúng chức năng; (2) bỏ test edge case mở rộng cho Nhánh 2, giữ số liệu FPR/AUC đã có; (3) không cắt: Nhánh 1 phải chạy đúng, demo end-to-end phải hoạt động, báo cáo phải nộp đúng hạn.
...
1. **Nhánh 3 (Session-level / Sequence Model) — toàn bộ.** Đây là đóng góp lý thuyết chính của đề tài (giải quyết khoảng trống về temporal query splitting mà Related Work bỏ qua) nhưng **chưa triển khai bất kỳ phần nào** tại 21/7: chưa Docker lab, chưa session data, chưa model. Cần: dựng lab (DVWA/WebGoat) → sqlmap thu traffic thật → gán nhãn 2 tầng → so sánh kiến trúc Tầng 2 (GRU/CNN/Transformer nhẹ) → train + đánh giá.
2. **Continual Learning đầy đủ** — pipeline gán nhãn từ hàng đợi Overkill → retrain có rehearsal → validation gate. Bản 25/7 chỉ có decision logic cơ bản (BLOCK/OVERKILL/ALLOW), không có vòng lặp học liên tục.
3. **Concept Drift monitoring production** — log định kỳ PSI/KL-divergence, FPR/Recall theo thời gian, versioning + rollback model.
4. **Session Store production-grade** (TTL/eviction, Redis) — cần thiết khi có Nhánh 3.
5. **Benchmark latency/throughput dưới tải thật** — bản 25/7 chỉ test chức năng đúng/sai, chưa đo throughput dưới tải cao.
6. **Adversarial hardening nhiều vòng** (WAF-A-MoLE lặp lại nhiều vòng sinh-test-retrain cho Nhánh 1).
7. **Sanity-check nhãn tay quy mô lớn** (~100+/lớp, kiểm định chéo nhiều người) — hiện mới soi mẫu nhỏ (15-30/lớp).
8. **Publish dataset chính thức** — license D1 (SQLiV3) chưa rõ ràng (xem `data_contract.md`), cần xác minh/thay thế trước khi trích dẫn rộng rãi.
9. **So sánh với nhiều baseline SOTA hơn** — cho phiên bản mở rộng (journal) nếu muốn nâng tầm công bố sau hội nghị.
