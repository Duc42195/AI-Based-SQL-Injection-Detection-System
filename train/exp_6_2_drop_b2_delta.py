"""exp_6_2_drop_b2_delta.py — Experiment 6.2: Drop B2 + delta features.

Remove B2 anomaly score from delta input.
Input: B1 probs deltas (5 dim) + timing gap (1 dim) = 6 dim.
...
B2_COL = 5  # index of B2 anomaly score in 7-dim vector
...
def drop_b2(X: np.ndarray) -> np.ndarray:
    """Remove B2 anomaly score column (index 5), keep B1 probs (0-4) + gap (6)."""
    cols = [c for c in range(X.shape[2]) if c != B2_COL]
    return X[:, :, cols].copy()


def load_transform_and_drop(data_dir: Path):
    ...
    X_train = drop_b2(compute_deltas(X_train))
    X_test = drop_b2(compute_deltas(X_test))
    logger.info("Delta + drop B2: train=%s test=%s", X_train.shape, X_test.shape)
...
def _build_session_vector(df, b1_scorer, cls_label, b2_model, max_decode, max_len):
    ...
    if arr.shape[0] >= max_len:
        ...
        vec = np.concatenate([pad, arr], axis=0)
    # apply delta + drop B2
    raw = vec[np.newaxis, :, :]
    return drop_b2(compute_deltas(raw))[0]
...
def run_zero_day_test(b2_model, raw_dir, max_len=64, max_decode=3, max_sessions=MAX_SESSIONS_PER_CLASS):
    ...
    for var_name, (b1_dir, target_label) in variants.items():
        logger.info("Zero-day %s — building delta+dropB2 features", var_name)
...
def main():
    ...
    logger.info("=" * 50)
    logger.info("Experiment 6.2 — Drop B2 + Delta Features")
    logger.info("=" * 50)

    logger.info("Step 1: Load, transform to delta, drop B2")
    X_train, y_train, X_test, y_test = load_transform_and_drop(data_dir)

    logger.info("Step 2: Train B3 on delta+dropB2 features")
    ...
    results = {
        "experiment": "6.2_drop_b2_delta",
        "description": "Remove B2 score, keep B1 deltas (5) + timing gap (1) = 6-dim",
        "config": {
    ...
    out_path = metrics_dir / "experiment_6_2_drop_b2_delta.json"
