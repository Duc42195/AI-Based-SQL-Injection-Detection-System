...
- Xử lý: thêm lưới lọc nội dung (`matches_any_attack_signature`, độc lập với nhãn nguồn) áp cho mọi dòng dự kiến `normal` trước khi chấp nhận. Loại được **1.561 dòng** (~5,6% pool `normal` ứng viên).
- Giới hạn còn lại: filter chỉ nhắm SQLi + OS command injection, **chưa phủ hết 12 loại tấn công của SR-BH** (vd. URL callback SSRF `owasp.org` vẫn có thể lọt) — chấp nhận được cho Nhánh 1 (chỉ cần SQLi), nhưng **Nhánh 2 (anomaly, cần benign pool sạch hơn nhiều) phải làm kỹ hơn** khi tới lượt (Ngày 5-6).
