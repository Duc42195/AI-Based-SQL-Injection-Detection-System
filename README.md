...
Muốn tự build lại từ đầu (raw → processed): xem `scripts/build_nhanh1_dataset.py`, `scripts/build_nhanh2_dataset.py`, `data_contract.md`.
...
Hoặc tự retrain (deterministic, seed=42): `uv run python scripts/train_nhanh1.py` (~15s) và
`uv run python scripts/build_nhanh2_data.py && uv run python scripts/train_nhanh2.py` (~75s).
