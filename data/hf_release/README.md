...
- **`branch1_train.csv`** — multi-class labels for **Branch 1** (supervised
  classifier). Labels go beyond binary normal/attack: each attack row is
  further tagged with its SQLi sub-technique.
- **`branch2_normal.csv`** + **`branch2_anomalous_eval.csv`** — benign-only pool
...
## Labels (Branch 1 only — `branch1_train.csv`)
...
### `branch1_train.csv` (Branch 1 — supervised multiclass)
...
### `branch2_normal.csv` / `branch2_anomalous_eval.csv` (Branch 2 — anomaly detection)
...
`branch2_normal.csv`) `split` (`train`/`test`, `test_size=0.2`, `seed=42`).

- **`branch2_normal.csv`**: 91,935 rows (73,548 train / 18,387 test), the full
...
- **`branch2_anomalous_eval.csv`**: 25,065 rows, D3's anomalous split, held out
...
| `d7_srbh2020` / `d7_srbh2020_normal` | SR-BH 2020 — a real honeypot capture (12 days, 2020) with multi-label CAPEC attack-type annotations, hosted on [Harvard Dataverse](https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/OGOIXX) | See the Dataverse page for the canonical citation. `_normal` suffix = rows the source labeled `Normal=1`, sampled (Branch 1) or fully used (Branch 2) and content-filtered (see below) |
| `d3_csic2010` | CSIC 2010 HTTP dataset — a widely-used synthetic e-commerce traffic capture, hosted on the [GSI/UdelaR GitLab mirror](https://gitlab.fing.edu.uy/gsi/web-application-attacks-datasets) | Used only in Branch 2 files (`branch2_normal.csv` / `branch2_anomalous_eval.csv`); URL + POST body extracted from the raw HTTP request blocks |
...
branch1 = load_dataset("Jason-42195/VNU-SQLi-Detection", data_files="branch1_train.csv")["train"]
print(branch1[0])
# {'id': 0, 'query_raw': "...", 'query_canonical': "...",
#  'has_comment_marker': 0, 'label': 3, 'label_name': 'boolean_blind',
#  'source': 'd7_srbh2020', 'split': 'train'}

branch2_normal = load_dataset("Jason-42195/VNU-SQLi-Detection", data_files="branch2_normal.csv")["train"]
branch2_eval = load_dataset("Jason-42195/VNU-SQLi-Detection", data_files="branch2_anomalous_eval.csv")["train"]
```
...
df = pd.read_csv("branch1_train.csv")  # or branch2_normal.csv
...
**Branch 2 metric:** false-positive rate on `branch2_normal.csv`'s test split +
detection rate on `branch2_anomalous_eval.csv` (keep in mind the eval set spans
