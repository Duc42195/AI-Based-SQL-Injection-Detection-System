...
| 2 | **Khối tác giả** — vùng highlight quanh `[TODO: dept] … International School, Vietnam National University, Hanoi` (tex 21–54) | *"Bỏ ':', có thể ghi 'including'."* | Đã rà soát và bỏ hết các câu ':' giữa câu ở trang 1 (Abstract, Introduction đoạn 1, Related Work) — nếu ý thầy là đúng chỗ này thì đã xong; riêng `[TODO: dept]` của Linh Dinh-Van vẫn là placeholder thật, cần tên khoa/viện thật mới điền được, không tự đoán | ☐ |
...
| 5 | **Introduction — đoạn thứ nhất** — tex 68: "Web applications store increasingly sensitive data… escalate privileges." | *"Đoạn thứ nhất này nên có trích dẫn."* | Thêm \cite{b12} (OWASP Top 10:2021 — A03 Injection) ngay ở câu "among the oldest and most severe such threats" | ✅ |
...
| 12 | **Related Work — câu có ':' giữa câu** (tex 85–88, đoạn "Traditional SQLi defenses rely on input validation…" và "ML-based detectors extract features from queries—bag-of-words, TF-IDF, or character/word n-grams—and train classifiers…") | *"Nên rà soát lại các câu này. Tại sao lại ':' ở giữa câu. Câu này nên tách thành các câu nhỏ hoặc viết đơn giản hơn cho dễ hiểu."* | Tách hết các câu ':' trong đoạn này; nhân tiện phát hiện và sửa lỗi dấu "-" bị dính chữ ("queries-bag-of-words") sót lại từ lần bỏ em-dash trước | ✅ |
| 13 | **Related Work — toàn đoạn** (tex 84–89) | *"Các related works đưa ra này phải phân tích được những ưu điểm và nhược điểm của nó, để từ đó làm nổi bật lên research gap mà mình cần giải quyết trong bài này là gì."* | Đoạn traditional defenses giờ nói rõ ưu (nhanh, dễ hiểu, không cần data huấn luyện) và nhược (rule cố định không lường trước được payload mới) trước khi dẫn sang ML | ✅ |
| 14 | **Related Work — đoạn traditional defenses** (tex 85) | *"Tìm thêm các bài về traditional SQLi defenses, phân tích qua 1 chút ưu điểm, nhược điểm của nó, sau đó kết luận phương pháp này không còn hiệu quả nên mới chuyển sang ML-based methods."* | Thêm \cite{b13} (Al-olaqi et al. 2025, survey về traditional + modern SQLi mitigation) | ✅ |
...
| 23 | ⚠️ **Phần Results nói chung** (tex 156–305) | *"Nhiều đoạn nên viết ngắn gọn hơn hoặc cắt bớt đi. Nếu có dùng ChatGPT để phân tích kết quả thì nên viết lại cho ngắn gọn hơn. Thế mới đủ được 6 trang đôi."* | Đã cắt câu chữ đáng kể ở tất cả 4 mục Results + bỏ hẳn hình ROC per-class (số liệu vẫn giữ nguyên trong text); giấy hiện còn 10 trang (từ 13), chưa đủ 6 — xem G7 | 🔄 |
...
| # | Vị trí | Comment của thầy | Ghi chú | Tiến độ |
|---|---|---|---|---|
| 25 | **Phần nội dung trang 8** | *"Phần này nên lồng ghép vào nội dung trên hoặc bỏ thì mới co lại đủ 6 trang đôi được."* | Discussion rút từ 10 mục còn 5: bỏ hẳn các mục lặp lại y nguyên Results/Setup (recon-stage variance, ablation-methodology caution, Session Correlator generalization gap, 1 câu filler cuối), gộp 2 mục về chi phí LocalOutlierFactor làm 1 | ✅ |

### Trang 9 — Conclusion

| # | Vị trí | Comment của thầy | Ghi chú | Tiến độ |
|---|---|---|---|---|
| 26 | **Conclusion and Future Work** (tex 326+) | *"Nên viết ngắn gọn hơn. Chỉ cần bảo paper này chúng tôi đề xuất gì, kết quả đạt được ra sao, future work thế nào."* | Viết lại ngắn gọn, không lặp lại từng con số đã có ở abstract; chỉ nói đề xuất — kết quả chính — future work | ✅ |
...
| G6 | **Tránh kiểu văn ChatGPT**: dấu `-`, in đậm, câu dài nhiều mệnh đề, dấu `:` và `/` giữa câu | Đã dọn hết dấu ':' giữa câu ở Abstract/Introduction/Related Work (chỗ thầy trỏ tới cụ thể); còn rải rác vài chỗ ':' ở System/Results/Discussion chưa rà hết toàn bài — mức độ ưu tiên thấp hơn vì không lặp lại nhiều lần như ở trang 1 | 🔄 |
| G7 | **Giới hạn độ dài: 6 trang đôi (IEEE)** | Từ 13 trang xuống còn 10 sau khi cắt chữ + bỏ 3 hình; để về đúng 6 trang cần cắt hẳn nội dung/kết quả, không chỉ câu chữ nữa — cần thầy/nhóm quyết định phần nào bỏ hẳn | 🔄 |
...
| A8 | Co paper về đúng 6 trang đôi. | #23, #25, #26, G7 | 🔄 (13→10 trang, chưa đủ 6) |