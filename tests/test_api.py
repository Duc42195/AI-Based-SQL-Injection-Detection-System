...
def test_train_job_lifecycle() -> None:
    ...
    status = client.get(f"/api/v1/train/branch1/status/{job_id}")
    ...
    for _ in range(80):
        s = client.get(f"/api/v1/train/branch1/status/{job_id}").json()
        if s["status"] == "done":
            break
        _time.sleep(0.2)
    result = client.get(f"/api/v1/train/branch1/result/{job_id}")
