...
This paper describes a multi-branch AI system that performs SQLi detection at the \emph{database proxy}, inspecting the fully-built SQL statement before it reaches the database server. Two query-level branches, a supervised multi-class classifier and a benign-only anomaly detector, feed a session-level correlation mechanism that catches blind and multi-step attacks neither branch sees alone. On a combined public dataset, the system reaches an F1-macro of 0.8977 for query-level classification and an AUC of 0.915 for anomaly detection, and a leave-one-out study shows the two branches together recover most attacks that either one misses individually. The contributions of this paper are:
...
Table~\ref{tab:b1arch} compares the four candidate architectures on the six-class dataset (Section~\ref{sec:setup}), all retrained with the retuned TF-IDF representation where applicable (Logistic Regression and LightGBM both use it; DistilBERT and the CNN use their own tokenizers and are unaffected by that change). DistilBERT reaches the highest F1-macro (0.965), with LightGBM close behind (0.962) at a similar latency (3.0~ms vs.\ 2.8~ms): the smaller vocabulary the TF-IDF retune produced (5{,}037 terms) cut LightGBM's per-query latency by $\approx$20$\times$ as a side effect, removing the reason it was ruled out for a real-time proxy. The CNN tokenizer (0.749) remains the weakest candidate by a wide margin; its small (37K-parameter) capacity does not hold up against the harder, evasion-heavy attack distribution D3 introduces. TF-IDF~+~Logistic Regression remains the deployed choice: at the retuned configuration it reaches 0.898, a 6.7-point gap to DistilBERT, at 0.5~ms latency and a 0.4~MB artifact, two orders of magnitude smaller than any other candidate. LightGBM is the more interesting alternative to weigh against it, within 0.3 points of DistilBERT's accuracy at a fraction of its size and training cost.

\begin{table}[t]
\caption{Branch~1 architecture comparison (6-class data, tuned TF-IDF config).}
\label{tab:b1arch}
\begin{center}
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\midrule
TF-IDF + LogReg (chosen) & 0.898 & 0.5~ms & 0.4~MB & 32~s \\
TF-IDF + LightGBM & 0.962 & 3.0~ms & 4.5~MB & 110~s \\
DistilBERT & 0.965 & 2.8~ms$^{\ast}$ & 256~MB & 1{,}970~s \\
...
The deployed TF-IDF configuration was carried over unchanged from an earlier, less evasive dataset revision (character n-grams (2,4), 50{,}000 features) and had never been re-tuned for data this hard; an initial run at that configuration reached only F1-macro~=~0.8352. A grid search over the n-gram range and Logistic Regression's regularization strength found that shorter character n-grams generalize better on this data, and the retuned configuration (n-grams (1,2), regularization $C=10$) reaches F1-macro~=~0.8977, the number reported above, while also shrinking the vocabulary from 50{,}000 to 5{,}037 terms.

Table~\ref{tab:b1perclass} gives the per-class breakdown at this tuned configuration. \texttt{error\_based}, \texttt{union\_based}, and \texttt{boolean\_blind} remain robust (F1~$\geq$~0.98), but \texttt{stacked} (F1~=~0.700) and \texttt{time\_blind} (F1~=~0.812) show a large, specific confusion with each other. This traces directly to D3's \texttt{recon}/\texttt{exploit} campaign structure (Section~\ref{sec:setup}): the \texttt{stacked} test split happens to draw from only 8 of D3's 45 \texttt{stacked} campaigns, a consequence of the campaign-level split required to avoid near-duplicate leakage, and those 8 are recon-heavy, so a small, unlucky campaign sample still dominates the aggregate. Per-class ROC curves (Fig.~\ref{fig:b1roc}), which measure ranking quality independent of the 0.5 decision threshold, stay high across all six classes (AUC~$\geq$~0.964) even where F1 is lower: the model still ranks \texttt{stacked}/\texttt{time\_blind} queries above benign ones reliably, so the confusion is specifically between the two attack sub-types, not between attack and benign. This F1 is measured on a clean test split, not on adversarially perturbed input, and should be read accordingly.

\begin{table}[t]
\caption{Branch~1 per-class results (test set, $n=18{,}295$), tuned TF-IDF config.}
...
\midrule
\texttt{normal}        & 0.863 & 0.965 & 0.911 & 3{,}000 \\
\texttt{union\_based}  & 0.990 & 0.981 & 0.986 & 3{,}027 \\
\texttt{error\_based}  & 0.995 & 0.990 & 0.993 & 3{,}033 \\
\texttt{boolean\_blind}& 0.982 & 0.986 & 0.984 & 3{,}016 \\
\texttt{time\_blind}   & 0.737 & 0.905 & 0.812 & 3{,}007 \\
\texttt{stacked}       & 0.860 & 0.590 & 0.700 & 3{,}212 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
\midrule
Content check  & 0/70 & 1.0 & 1.0 & 0.986 \\
Behavior check & 0/70 & 1.0 & 1.0 & 0.457 \\
Combined (OR)  & 0/70 & 1.0 & 1.0 & 0.986 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
