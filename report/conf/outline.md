...
- **D. Branch 2 — query-level anomaly** — ✍️ One-Class SVM / Isolation Forest on statistical features. **Updated 16/08: feature set revised to 3 features (`sql_keyword_count`, `entropy`, `bigram_entropy` — replacing `length`/`special_char_ratio`, dropped after a benign-pool cleanup made `length` a confound instead of a signal; see `full_outline.md` §2.5).** Writable now; new numbers in `full_outline.md` §2.9 (AUC 0.902→0.949, DR-at-matched-FPR=5% 41.9%→31.2%(regression)→63.6%(recovered)). The cleanup→regression→feature-fix arc is itself a good methodology talking point (measured, not assumed) if word count allows.
...
- **C. Branch 2 dataset** — ⚠️ this figure (91,935 benign train / 25,065 anomalous eval) is from the original raw-source pipeline (`train/build_branch2_dataset.py`) and does NOT match what actually produced the 16/08 results (`train/build_branch2_data.py`, the HF-hosted pool: 9,164 benign train / 26,218 anomalous eval post-cleanup). Needs Duc to say whether this bullet should switch to the HF-pool numbers or whether the 91,935-row experiment is being kept as a separate reported figure — see the same flag in `full_outline.md` §2.5.
