...
size_categories:
- 10K<n<100K
pretty_name: SQLi Multiclass Detection Dataset
---

# SQLi Multiclass Detection Dataset

A multi-class SQL Injection detection dataset combining 3 public sources plus a
small synthetic supplement, built for training the supervised classifier
("Branch 1") of a 3-branch AI-based SQL Injection detection system. Labels go
beyond binary normal/attack: each attack row is further tagged with its SQLi
sub-technique.
...
ds = load_dataset("Jason-42195/VNU-SQLi-Detection")

train = ds["train"]  # or filter the `split` column if loaded as a single CSV config
print(train[0])
...
If loaded as a single CSV file rather than a configured `train`/`test` split, filter manually:

```python
import pandas as pd

df = pd.read_csv("nhanh1_train.csv")
train_df = df[df["split"] == "train"]
test_df = df[df["split"] == "test"]
```

Recommended primary metric: **F1-macro** (not accuracy) — the original per-source
class sizes were extremely imbalanced (see Dataset Structure) before undersampling,
and `stacked` remains a tiny synthetic minority class even after balancing.
